...
public class ChangingCustomerCompanySettingByAdmin implements Command {
	private static final Logger log = LoggerFactory.getLogger(ChangingCustomerCompanySettingByAdmin.class.getName());
	...
	public void execute(final HttpServletRequest request, final HttpServletResponse response)
		...
		selectedUser = new User.Builder().setUsername(selectedUser.getUsername())
				...
				.setAddress(address).setPhoto(photo).setRole(role).setPerson(person).setCustomer(customerCompany).build();
		...
		try {
			userService.editCustomerCompany(selectedUser, locale);
