...
public class ChangingCustomerLegalSetting implements Command {
	private static final Logger log = LoggerFactory.getLogger(ChangingCustomerLegalSetting.class.getName());
	...
	public void execute(final HttpServletRequest request, final HttpServletResponse response)
		...
		user = new User.Builder().setUsername(user.getUsername()).setIsDelete(ControllerConstant.USER_FALSE_PARAM_NAME)
				...
				.setPerson(person).setCustomer(customerLegal).build();
		...
		try {
			userService.editCustomerLegal(user, locale);
