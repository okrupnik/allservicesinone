...
public class CreateOrderRepair implements Command{
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Specialization specialization = (Specialization) request.getSession(true).getAttribute(ParamAndAttribute.SPECIALIZATION_ATTRIBUTE);
		...
		startDate = LocalDate.parse(request.getParameter(ParamAndAttribute.START_DATE_PARAM_NAME), formatter);
		...
		orderRepair = new Order.Builder().setTitle(titleOrder).setIdSpecialization(idSpecialization).setDescription(description).setStatus(status).setSubtypeSpecialization(subspecialization).setStartDate(startDate).setDateOfCreating(dateOfCreating).setAddress(address).setAttachment(attachment).build();
