...
public class ConnectionPool {
	...
	static {
		try {
			...
			log.error("Can't create connection pool");
			throw new RuntimeException("Can't create connection pool");
		}
	}

	private BlockingQueue<Connection> connectionPool;
	private BlockingQueue<Connection> givenAwayConPool;
	...
	public void initPoolData() throws ClassNotFoundException, SQLException {
		...
		givenAwayConPool = new ArrayBlockingQueue<Connection>(poolSize);
	...
	public Connection takeConnection() throws InterruptedException {
		...
		givenAwayConPool.add(connection);
	...
	private void clearConnectionPool() throws SQLException {
		...
		deletedConnectoions.addAll(connectionPool);
		connectionPool.clear();
		deletedConnectoions.addAll(givenAwayConPool);
		givenAwayConPool.clear();

		closeConnectionsPool(deletedConnectoions);

	}

	private void closeConnectionsPool(List<Connection> pool) throws SQLException {
	...
	private class PooledConnection implements Connection {
		...
		public void close() throws SQLException {
			...
			if (!givenAwayConPool.remove(this)) {
